...
const Drawer = ({ onClose, items = [], onRemove, setCartItems, opened }) => {
  ...
  const onClickOrder = async () => {
    try {
      const res = await axios.post('https://60f2d6c76d44f300177887b8.mockapi.io/orders', {
        items: cartItems,
      })
  ...
  return (
    <div className={`${s.overlay} ${opened && s.overlayVisible}`} onClick={onClose}>
      <div
        className={s.drawer}
        onClick={e => {
          e.stopPropagation()
      ...
      >
        <h2 className={s.cartTitle}>
          Корзина <img alt="Close" src={removeBtn} onClick={onClose} />
        </h2>
        {items.length !== 0 ? (
          <div className={s.main}>
            ...
            <ul className={s.cartTotalBlock}>
              ...
              <button onClick={onClickOrder} className={s.btn_order}>
                Оформить заказ
                <img src={arrowRight} alt="Arrow" className={s.orderArrow} />
              </button>
            </ul>
          </div>
        ) : isOrderCompleted ? (
